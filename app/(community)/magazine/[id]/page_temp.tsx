...
export default async function Page({ params }: { params: { id: string } }) {
    ...
    const PUBLIC_BUCKET_URL = String(process.env.NEXT_PUBLIC_BUCKET_URL);
    return (
        <article className="container relative max-w py-6 lg:py-10">
            ...
            <div>
                ...
                <h1 className="mt-2 inline-block text-4xl font-extrabold leading-tight text-slate-900 lg:text-5xl">{detail?.magazine.feedTitle}</h1>
                {detail?.magazine ? (
            ...
            <div className="flex justify-center py-6 lg:py-10">
                <Link href="/magazine" className="inline-flex items-center justify-center text-sm font-medium text-slate-600 hover:text-slate-900">
                    <Icons.chevronLeft className="mr-2 h-4 w-4" />
                    See all magazines
                </Link>
            </div>
